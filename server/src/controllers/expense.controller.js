...
import { recordExpenseEntry } from "../services/accounting.service.js";
// Add the expenses
...
export const updateExpense = asyncHandler(async(req, res)=>{
    const { id } = req.params;
    const { title, amount, note, catagory, date } = req.body;
    const updatedExpense = await Expense.findByIdAndUpdate(id, { title, amount, note, catagoryId: catagory, expensesDate:date }).populate('catagoryId');
    if (!updatedExpense) {
        return res.respond(404, "Expense not found");
    }
